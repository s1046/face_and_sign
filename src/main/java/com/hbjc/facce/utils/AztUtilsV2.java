...
public class AztUtilsV2 {
    ...
    static {
        ...
        ICLOUD_DOC_PREFIX = environment.getProperty("azt.v2.aztUrl");
        APP_ID = environment.getProperty("azt.v2.appId");
        APP_SECRET = environment.getProperty("azt.v2.appSecret");
        NOTIFY_URL = environment.getProperty("azt.v2.notifyUrl");
        VERSION = environment.getProperty("azt.v2.version");
        JSON_FILE_PATH = environment.getProperty("azt.v2.jsonFilePath");
    ...
    public static String readDebugJson(String relativePath) throws URISyntaxException, IOException {
        if (StringUtils.isEmpty(JSON_FILE_PATH)) {
            ...
            return FileUtils.readFileToString(new File(AztUtilsV2.class.getClassLoader().getResource(relativePath).toURI().getPath()));
        }
